...
class extra_c_loss(_Loss):
	def __init__(self, model, learn_epochs, total_epochs, use_cuda=False, cuda_device=None, 
		...
		super(extra_c_loss, self).__init__()
		self.model = model
		#self.alpha = alpha
	...
	def __call__(self, input_batch, target_batch, epoch):
		if epoch <= self.learn_epochs or not self.model.training:
			...
			if self.model.training:
				...
				h_c = F.nll_loss(F.log_softmax(input_batch, dim=1), target_batch, reduction='none')
			...
			#F.nll_loss(F.log_softmax(logits_classifier, dim=1), target, reduction='none')
			#h_c = F.cross_entropy(input_batch,target_batch,reduce=False)
			h_c = F.nll_loss(F.log_softmax(input_batch, dim=1), target_batch, reduction='none')
			p_out = torch.exp(F.log_softmax(input_batch,dim=1))
			#probabilities of abstention  class
			p_out_abstain = p_out[:,-1]

			_, pred = input_batch.topk(1, 1, True, True)
			pred = pred.t()
			correct = pred.eq(target_batch.reshape(1, -1).expand_as(pred)).float()
			...
			try:
				...
				if self.alpha_thresh_ewma is None:
					...
					self.alpha_thresh_ewma = Variable(self.ewma_mu*self.alpha_thresh.data + \
						(1. - self.ewma_mu)*self.alpha_thresh_ewma.data)
				"""
				self.alpha_thresh_ewma = Variable(torch.tensor(1.0))
				...
				pred_out_bound = (1 + self.alpha_var * 1 / (1 + self.alpha_var * p_out_abstain)) * (1 - correct) + correct
				loss = (h_c * pred_out_bound).mean()
				#loss = h_c + self.alpha_var * torch.log(p_out_abstain) / (1 + self.alpha_var*p_out_abstain) 

...
def get_optimizer_fine_tune(model: "nn.Module", **kwargs) -> "Tuple[optim, Tuple[lr_scheduler]]":
    """
    Get model optimizer
    ...
    """
    name = kwargs["name"]
    lr = kwargs["lr_finetune"]
    weight_decay = kwargs["weight_decay"]
    decay_frequency = kwargs["decay_frequency_finetune"]
    decay_factor = kwargs["decay_factor_finetune"]
...
class get_lr(object):
    def __init__(self, lr_ini, factor, divid_p):
        self.lr_init = lr_ini 
        self.factor = factor  
        self.divid_p = divid_p
        self.swa_start = 200
        self.epochs = 250
        self.swa = True
        self.swa_lr = 0.01
    ...
    def lr(self, epoch):
        t = (epoch) / (self.swa_start if self.swa else self.epochs)
...
class get_lr(object):
    ...
    def lr(self, epoch):
        for i in range(1, len(self.divid_p)):
            if epoch > self.divid_p[i-1]:
                lr = self.lr_ini * self.factor**i
                return lr
"""
...
def save_weights(
    ...
    model_path = Path(ckpt_dir) / "model_swa.pth"
    model_path_m = Path(ckpt_dir) / "model_swa.ckpt"
    
    torch.save(state_dict["model"], model_path_m)
